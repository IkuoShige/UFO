...
### 単純 PD で立位を保持することはできない

デフォルト姿勢を関節 PD だけで保持すると**前方に倒れる**（歩行ゲインで 1.62 s、
get-up デプロイゲインで 1.19 s）。足首まわりの倒立振子としての不安定化剛性は
`m·g·h ≈ 19.67 × 9.81 × 0.5 ≈ 96.5 N·m/rad`、対する足首ピッチ総剛性は
歩行 `2×50 = 100`、UFO の get-up ゲイン `2×35.7 = 71.4`、
`k1_constants_isaaclab_getup.hpp` の 0.6× ゲイン `2×30 = 60`。
いずれも余裕が無いか下回る（総質量 19.666 kg は MJCF から独立に確認済み）。

`GETUP_DEFAULT_ANGLES`（`k1_constants_isaaclab_getup.hpp:48-55`）は
config 既定の脚角度そのものなので、**行動出力がゼロ付近で終わる get-up policy は
間違った姿勢に着地し、しかも最も柔らかいゲインでそれを保持する**ことになる。

### 引き継ぎ手順（上記を踏まえて修正）

「姿勢を寄せてからゲインをランプ」という素朴な順序は**成立しない** —
途中に「PD で姿勢を保持する」フェーズを置けないため。正しくは:

1. UFO ノードで起き上がり（`getup` z、0.64 s）
2. **BFM が能動的にバランスを取り続けたまま**、脚を実測目標姿勢へ寄せる
   （z 二相化 / z スケジュール。PD で固めて待つのは不可）
3. 姿勢が寄った時点で、**数百 ms 以内に** locomotion policy が受け取る
   （ゲインのランプもこの窓の中で行う）
4. LowCmd の発行権を locomotion ノードへ移す

つまり引き継ぎは「静止状態の受け渡し」ではなく、
**能動制御から能動制御への短い窓での受け渡し**として設計する必要がある。
